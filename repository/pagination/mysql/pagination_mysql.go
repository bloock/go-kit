...
func GetPaginationPgx(ctx context.Context, db *pgxpool.Conn, sb *sqlbuilder.SelectBuilder, pq pagination.PaginationQuery) (pagination.Pagination, error) {
	paginationSQLStruct := sqlbuilder.NewStruct(new(paginationResponse))
	...
	rows, err := db.Query(ctxTimeout, query, args...)
	if err != nil {
		return pagination.Pagination{}, err
	}
	defer rows.Close()
